...
int evsvrloop(int sockfd)
{
    ...
    for (;;)
    {
        ...
        read(sockfd, &bufflen, sizeof(bufflen));
        ...
        pkt->data[0] = '#';
        pkt->data[1] = '#';
        // Reuse pkt->data to reply Client. Reverse String
        for (i = 0; i < len_cltmsg; i++)
        {
            pkt->data[i + 2] = buff[len_cltmsg - 1 - i];
        }
        write(sockfd, &pkt->len, sizeof(uint16_t));
        write(sockfd, pkt->data, pkt->len);
        // if msg contains "q" then server exit and chat ended.
        if (strncmp("q", buff, 1) == 0)
        {
            printf("Exit command received. Exiting Server\n");
            break;
        }
    }
    return 0;
